...
export const ventasRepo = {
  ...
  async create({ cliente_id, fecha, notas, total, items, createdBy }) {
    ...
    try {
      ...
      await conn.query(
        `INSERT INTO ventas (id, folio, cliente_id, fecha, notas, total, createdBy, createdAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [venta.id, venta.folio, venta.cliente_id, venta.fecha, venta.notas, venta.total, venta.createdBy, venta.createdAt],
      )
  ...
  async update(id, { cliente_id, fecha, notas, total, items }) {
    ...
    try {
      await conn.beginTransaction()
      await conn.query('UPDATE ventas SET cliente_id=?, fecha=?, notas=?, total=? WHERE id=?', [cliente_id ?? null, fecha, notas ?? null, total, id])
