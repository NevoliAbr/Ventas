...
export async function createUnidad(req, res) {
  ...
  res.status(201).json({ unidad: await unidades.create({ nombre: nombre.trim(), abreviatura: abreviatura?.trim() || null }) })
}
export async function updateUnidad(req, res) {
  const { nombre, abreviatura } = req.body ?? {}
  if (!nombre?.trim()) return res.status(400).json({ error: 'El nombre es obligatorio.' })
  const u = await unidades.update(req.params.id, { nombre: nombre.trim(), abreviatura: abreviatura?.trim() || null })
