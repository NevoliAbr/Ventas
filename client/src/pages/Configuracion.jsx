      ...
      <div className="slds-container_large slds-container_center">
        ...
        {!perms.canViewUsers ? (
          ...
          <div className="slds-box slds-theme_default">
            ...
            {tab === 'usuarios' && <UsuariosTab perms={perms} user={user} />}
            {tab === 'correo' && perms.canEditUsers && <CorreoTab />}
          </div>
        )}
      </div>
    </div>
...
function UsuariosTab({ perms, user }) {
...
function CorreoTab() {
  ...
  useEffect(() => {
    if (showLinkBox) linkInputRef.current?.focus()
  }, [showLinkBox])
    ...
    <div style={{ maxWidth: '680px' }}>
      ...
      <form onSubmit={enviarCorreo}>
        ...
        <div className="slds-form-element slds-m-bottom_medium">
          <label className="slds-form-element__label" id="correo-mensaje-label" htmlFor="correo-editor">
            <abbr className="slds-required" title="required">*</abbr> Mensaje
          </label>

          {/* Toolbar */}
          <EditorToolbar
            execCmd={execCmd} saveSelection={saveSelection}
            currentColor={currentColor} colorRef={colorRef} onApplyColor={applyColor}
            showLinkBox={showLinkBox} onToggleLinkBox={() => {
          ...
          {showLinkBox && (
            <div style={{
              ...
              />
              <button type="button" onClick={insertLink} className="correo-link-insertar-btn"
              >Insertar</button>
          ...
          <div
            ...
            role="textbox"
            aria-multiline="true"
            aria-labelledby="correo-mensaje-label"
            onBlur={saveSelection}
            className="correo-editor-body"
          />

          {/* Vista previa de la firma */}
          <div className="correo-firma-preview">
          ...
          {adjuntos.length > 0 && (
            <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {adjuntos.map((f, i) => (
                <span key={`${f.name}-${f.size}-${f.lastModified}`} className="correo-adjunto-chip">
                  📎 {f.name}
...
function EditorToolbar({ execCmd, saveSelection, currentColor, colorRef, onApplyColor, showLinkBox, onToggleLinkBox, fileRef, onAdjuntar }) {
  return (
    <div className="correo-editor-toolbar">
      ...
      >
        <span style={{ ...tbBtn, flexDirection: 'column', gap: '1px' }}>
      ...
      <button type="button" title="Alinear derecha" aria-label="Alinear derecha"
      ...
      {/* Hipervínculo */}
      <button type="button" title="Insertar enlace" aria-label="Insertar enlace"
        onMouseDown={(e) => { e.preventDefault(); onToggleLinkBox() }}
      ...
      {/* Adjuntar archivo */}
      <button type="button" title="Adjuntar archivo" aria-label="Adjuntar archivo"
